...
2. Run `prepare RELEASE_ID`. It matches Anikoto using both English and romaji names, checks season and episode, then downloads and decompresses the English ASS. Ambiguous matches, split cours, unavailable episodes and attachments remain queued for another run or user review. Never lower thresholds or infer numbering offsets. Prepare only queued items; ready items retain their translation work.
